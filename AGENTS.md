...
- **`firmware-idf/waveshare-amoled/`** — third board: Waveshare ESP32-S3 round
  ...
  `m5ct` protocol over **both USB-Serial/JTAG and BLE** (NimBLE, `main/ble.c`,
  same GATT contract as CoreS3 — no host logic changes; needs the
  `@abandonware/noble` optional dep). See its `README.md`.
...
## Status (as of 2026-05-30 — verify against git before relying on it)

The status-display reposition, liveness redesign, release packaging, host-screenshot, tap command, activity-badge, RTC time sync, and multi-session picker work are all merged to `main` and HW-verified on CoreS3 SE. Milestones M0–M5 are V1 (M6/M7 = V1.x: BLE/WS transport, tmux prompt injection).

Versions are not tracked here (they rot). The truth lives in source: CLI in `packages/cli/package.json`, firmware `fw_ver` in `firmware/boards/cores3_se/board.cpp` and the pinned default in `packages/cli/src/firmware-index.ts`. Firmware releases are `fw-<board>-<ver>` git tags — run `git tag -l 'fw-*'` for the published set.
...
- **npm publish not done** — verify the `m5ct` name is free (`npm view m5ct`, fallback `@m5stack-coding-toys/m5ct`), set repo secret `NPM_TOKEN`, then a `v*` tag (`git tag v<x.y.z> && git push --tags`) triggers `.github/workflows/release.yml`. (Firmware releases are separate — see the `fw-<board>-<ver>` flow, already in use.)
- **`m5ct install` real auto-start** — launchd-plist generation still deferred (install writes settings.json but not a launch agent).
- **Reconnect robustness** — on supersede/open-failure the `DevicePoller.seen` set is never cleared, so a device that fails to open isn't re-attempted until replug; a daemon was also once observed stuck in Scanning after a failed hello. Both are pre-existing device-manager gaps left for a hardware-in-hand session. (A related case — a *half-open* link stuck in `Connected` where pings time out forever, e.g. a zombie BLE GATT connection that never fires `close` — is now handled: `DeviceSession` force-disconnects after `maxMissedPings` consecutive ping timeouts so the manager re-scans.)
