...
- **Flashing your worktree firmware** (`pio run -e cores3-se -t upload`): send the daemon `{op:'flashHold',client:'x'}` over `~/.m5stack-coding-toys/daemon.sock` (it closes the port + goes `Held`, auto-releases after 60s), upload, then `{op:'flashRelease',client:'x'}`. `m5ct flash` only flashes *released* firmware, not your build. CoreS3 auto-reset is a no-op → ask the human to press RESET after upload.
