...
const waitFor = (pred, ms) =>
  new Promise((resolve, reject) => {
    ...
    handlers.push((e) => { if (pred(e)) { clearTimeout(to); resolve(e) } })
  })
...
async function shot(name) {
  ...
  if (!e.p.ok || !e.p.data_b64) { console.error(name, 'capture failed', e.p); return }
...
sp.on('open', async () => {
  try {
    send({ v: 1, k: 'hello', t: Date.now(), p: { caps: ['display', 'notify'], time: { utc_ms: Date.now(), offset_min: 600 } }, id: 'h1' })
    await waitFor((e) => e.k === 'hello.ack', 4000)

    // 1. idle / linked
    send({ v: 1, k: 'status', t: Date.now(), p: { state: 'idle' }, id: 'st0' })
    await sleep(600); await shot('01_idle')

    // 2. live working
    send({ v: 1, k: 'status', t: Date.now(), p: LIVE, id: 'st1' })
    await sleep(700); await shot('02_live_working')

    // 3. awaiting input (amber)
    send({ v: 1, k: 'status', t: Date.now(), p: { ...LIVE, activity: 'awaiting_input', context: { usedPct: 73, tokens: 146000, limit: 200000, exceeds200k: false } }, id: 'st2' })
    await sleep(700); await shot('03_awaiting')

    // 4. needs attention (red, high ctx)
    send({ v: 1, k: 'status', t: Date.now(), p: { ...LIVE, activity: 'needs_attention', context: { usedPct: 94, tokens: 188000, limit: 200000, exceeds200k: false } }, id: 'st3' })
    await sleep(700); await shot('04_attention')

    // 5. notify overlay
    send({ v: 1, k: 'notify', t: Date.now(), p: { title: 'Permission needed', body: 'Allow edit to proto.c?', urgency: 'high' }, id: 'nt1' })
    await sleep(700); await shot('05_notify')

    console.error('done')
    process.exit(0)
  } catch (e) { console.error('ERR', e.message); process.exit(2) }
})
sp.on('error', (e) => { console.error('PORT ERR', e.message); process.exit(3) })
